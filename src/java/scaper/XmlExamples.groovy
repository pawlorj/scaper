...
class XmlExamples {
    ...
    <records>
      <car name='HSV Maloo' make='Holden' year='2006'>
        ...
        <record type='speed'>Fidelity Advisor\u00ae Leveraged Co Stk Z</record>
      </car>
  ...
  def test() {
	  ...
	  def records = slurper.parseText(XmlExamples.CAR_RECORDS)
	  println CAR_RECORDS
	  def c = records.car.find{ it.@make.text().contains('Holden') }
	  return c.record.text().trim()
  }
